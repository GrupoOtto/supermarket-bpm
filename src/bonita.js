...
exports.initiate = credentials => async (processDefinitionId, variables) => {
    const response = await axios.post("http://localhost:8080/bonita/API/bpm/case", {
        processDefinitionId,
        variables
    }, {
        headers: {
            Cookie: _.toPairs(credentials).map(cookie => cookie.join("=")).join("; "),
